...
var _ = Describe("main", func() {
	It("wires the expected FX modules", func() {
		...
		Expect(modules).To(HaveLen(8))
		Expect(modules[0]).To(Equal(appfx.ConfigModule))
		Expect(modules[1]).To(Equal(appfx.LoggerModule))
		Expect(modules[2]).To(Equal(appfx.MetricsModule))
		Expect(modules[3]).To(Equal(appfx.AppModule))
		Expect(modules[4]).To(Equal(appfx.StorageModule))
		Expect(modules[5]).To(Equal(appfx.RepoModule))
		Expect(modules[6]).To(Equal(appfx.ServiceModule))
		Expect(modules[7]).To(Equal(appfx.PresentationModule))
	})
})
